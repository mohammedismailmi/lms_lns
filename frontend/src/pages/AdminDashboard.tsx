    ...
    return (
        <div className="p-8 max-w-[1600px] mx-auto space-y-10 font-sans pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Link to="/admin/instructors" className="block hover:scale-[1.02] transition-transform">
                    <StatCard title="Instructors" value={stats?.totalInstructors || 0} icon={<UserCheck />} />
                </Link>
                <Link to="/admin/learners" className="block hover:scale-[1.02] transition-transform">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {sessions.length === 0 ? (
                        ...
                        <div 
                            ...
                            className="p-4 rounded-lg border border-slate-100 bg-slate-50 relative overflow-hidden group hover:border-primary/30 transition-colors cursor-pointer"
                        >
                            ...
                            <h3 className="font-bold text-navy mb-1 line-clamp-1 group-hover:text-primary transition-colors">{session.title}</h3>
                            {(session as any).description && (
                                <p className="text-[11px] text-muted mb-3 line-clamp-2 italic">{(session as any).description}</p>
                            )}
            ...
            {selectedSession && (
                <div className="fixed inset-0 bg-navy/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-border">
                        <div className="p-6 border-b border-border flex items-center justify-between bg-surface/50">
                            <div>
                                <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">{selectedSession.course_title}</p>
                                <h3 className="text-xl font-serif font-bold text-navy">{selectedSession.title}</h3>
                            </div>
                            <button onClick={() => setSelectedSession(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-muted hover:text-navy">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="p-8 space-y-6">
                            <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="space-y-1">
                                    <p className="text-xs text-muted uppercase font-bold tracking-wider">Date</p>
                                    <p className="text-navy font-bold flex items-center gap-2">
                                        <Users className="w-4 h-4 text-primary" />
                                        {new Date(selectedSession.start_time * 1000).toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}
                                    </p>
                                </div>
                                <div className="w-px h-10 bg-slate-200" />
                                <div className="space-y-1">
                                    <p className="text-xs text-muted uppercase font-bold tracking-wider">Time</p>
                            ...
                            <div className="space-y-2">
                                <p className="text-xs text-muted uppercase font-bold tracking-wider">Description</p>
                                <div className="p-4 bg-surface/50 rounded-xl border border-border min-h-[100px] text-sm text-navy/80 leading-relaxed italic">
                            ...
                            {selectedSession.meet_link ? (
                                <div className="space-y-3 pt-2">
                                    <p className="text-xs text-muted uppercase font-bold tracking-wider">Meeting Access</p>
                                    <a 
                                        href={selectedSession.meet_link} 
                                        target="_blank" 
                                        rel="noopener noreferrer"
                                        className="flex items-center justify-center gap-3 w-full py-4 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all hover:bg-primary-dark"
                                    >
                                        <Video className="w-5 h-5" /> Join Live Meeting
                                    </a>
                                    <p className="text-[10px] text-center text-muted">Join link will open in a new window</p>
                                </div>
                            ) : (
                                <div className="p-4 bg-amber-50 rounded-xl border border-amber-100 flex items-center gap-3 text-amber-700 text-sm italic">
                                    <ShieldBan className="w-5 h-5" /> No meeting link available for this session.
                                </div>
                            )}
                        </div>
                    </div>
